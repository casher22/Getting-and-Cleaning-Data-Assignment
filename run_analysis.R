##read each text file into R using a space as the separator
        ...
        activitylabels<- read.table("activity_labels",sep = "")
...
##create a new data set that groups and summarizes the variables by subject and activity
        summarizeddata<- combineddata %>% group_by(subject,activity) %>% summarise_each(funs(mean))
##check- summaizeddata dimensions should be 180 rows and 81 columns
